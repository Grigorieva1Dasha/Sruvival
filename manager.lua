...
function Manager:runToEat()
    if self.humanid == nil then
        return
    end
    for k, v in pairs(self.objects) do
        if k == self.humanid then
            human = v
        end
        self.nbacterium = self.nbacterium + 1
    end
    if #self.foods > 1 then
        ...
        if self.nbacterium == 1 then
            dp = dist:norm2()
            dp = dp * -5
            human:applyForce(dp)
        end
    end
    print(self.nbacterium)
    distd = Vector:create(10000,10000)
    for k_, v_ in pairs(self.objects) do
        if self.humanid ~= k_ then
